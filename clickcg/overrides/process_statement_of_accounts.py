...
class ClickcgProcessStatementOfAccounts(ProcessStatementOfAccounts):
    ...
    if TYPE_CHECKING:
        ...
        from erpnext.accounts.doctype.psoa_cost_center.psoa_cost_center import (
            PSOACostCenter,
        )
    ...
    def validate(self):
        ...
        if not self.body:
            ...
            self.body = (
                "Hello {{ customer.customer_name }},<br>PFA your Statement Of Accounts"
                + body_str
            )
...
def get_html(doc, filters, entry, col, res, ageing):
    ...
    template_path = (
        ...
        if doc.report == "General Ledger"
        else "clickcg/overrides/accounts_receivable.html"
    )
    if doc.letter_head:
